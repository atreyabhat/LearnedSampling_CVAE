...
base_directory = 'map_data/n_link'

# Get all folders named map_x_omni
map_folders = glob.glob(os.path.join(base_directory, 'map_*_nlink'))

# Output file for combined CSV contents within the 'map_data' folder
output_file = os.path.join(base_directory, 'Complete_training_data_nlink_50maps.csv')
...
with open(output_file, 'w', newline='') as outfile:
    ...
    for folder in map_folders:
        ...
        samples_files = glob.glob(os.path.join(folder, f'map_{map_number}_nlink_samples.csv'))  # Adjust pattern as needed
        for file_name in samples_files:
            if os.path.isfile(file_name) and file_name.endswith('.csv'):
                with open(file_name, 'r') as infile:
                    reader = csv.reader(infile)
                    for row in reader:
                        # Check if the row is not empty before inserting the map number
                        if any(row):
                            # Insert the map number at the beginning of each non-empty row
