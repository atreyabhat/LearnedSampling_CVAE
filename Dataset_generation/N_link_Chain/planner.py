...
class Planner:
    ...
    def plan(self, start, goal):
        ...
        self.start = start
        self.goal = goal
        self.solution, samples = self.method.plan(self.start, self.goal)
    ...
    def visualize(self, make_video=False):
        ...
        if self.solution != []:
            if make_video:
                ...
                metadata = dict(title='Movie Test', artist='Matplotlib', comment='An animation of the robot planning')
        ...
        plt.pause(0.1)  # Pause before closing the plot
        plt.close(fig)
